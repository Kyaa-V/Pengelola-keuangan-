...
function createData(icon: string, name: string) {
  return { icon, name };
}

const datas = [
  createData("home", "Home"),
  createData("group", "Profile"),
  createData("circle-three-quarter", "Analyst"),
  createData("wallet", "Transaksi"),
  createData("credit-card", "Data Piutang"),
  createData("purchase-tag-alt", "Data Transaksi"),
];
...
<template>
  ...
  >
    <div class="p-2 flex flex-col gap-2">
      <div class="relative flex items-center mb-4 mt-5 mr-2 justify-center">
        <span
          v-if="isOpen"
      ...
      >
        <box-icon
          ...
          :color="thema === 'day' ? '#000' : '#fff'"
        ></box-icon>
        <span v-if="isOpen" class="whitespace-nowrap">{{ data.name }}</span>
      </div>

      <div
        class="flex items-center gap-3 p-2 rounded-md cursor-pointer transition-all"
        :class="
          thema === 'day' ? 'bg-black text-white' : 'bg-gray-200 text-black'
        "
        @click="changeThema"
      >
        <box-icon
